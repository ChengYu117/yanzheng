...
from src.nlp_re_base.contrastive_gemini_io import validate_explainer_outputs
...
def _safe_smoke_root() -> Path:
    root = (Path.cwd() / "outputs" / "_smoke_contrastive_gemini_io").resolve()
...
def test_contrastive_gemini_io() -> None:
    ...
    try:
        ...
        tasks = [json.loads(line) for line in (root / "ide_tasks" / "explainer_tasks.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]
        ...
        validation = validate_explainer_outputs(
            tasks_path=root / "ide_tasks" / "explainer_tasks.jsonl",
            output_dir=root,
        )
        ...
        scorer_tasks = [json.loads(line) for line in (root / "ide_tasks" / "scorer_tasks.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]
        baseline_tasks = [json.loads(line) for line in (root / "ide_tasks" / "baseline_tasks.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]

        perfect_predictions = [
            {"sample_id": "h001", "pred_activate_prob": 0.95, "binary_prediction": 1, "evidence_span": "What would help", "reason": "matches open invitation"},
            {"sample_id": "h002", "pred_activate_prob": 0.85, "binary_prediction": 1, "evidence_span": "How could", "reason": "matches open invitation"},
            {"sample_id": "h003", "pred_activate_prob": 0.20, "binary_prediction": 0, "evidence_span": "Do you want", "reason": "closed confirmation"},
            {"sample_id": "h004", "pred_activate_prob": 0.10, "binary_prediction": 0, "evidence_span": "What happened", "reason": "too generic"},
        ]
        weak_baseline = [
            {"sample_id": "h001", "pred_activate_prob": 0.50, "binary_prediction": 0, "evidence_span": "", "reason": "unclear"},
            {"sample_id": "h002", "pred_activate_prob": 0.50, "binary_prediction": 0, "evidence_span": "", "reason": "unclear"},
            {"sample_id": "h003", "pred_activate_prob": 0.50, "binary_prediction": 0, "evidence_span": "", "reason": "unclear"},
            {"sample_id": "h004", "pred_activate_prob": 0.50, "binary_prediction": 0, "evidence_span": "", "reason": "unclear"},
        ]
        ...
        scorer_validation = validate_scorer_outputs(
            scorer_tasks_path=root / "ide_tasks" / "scorer_tasks.jsonl",
            baseline_tasks_path=root / "ide_tasks" / "baseline_tasks.jsonl",
...
if __name__ == "__main__":
    test_contrastive_gemini_io()
    print("test_contrastive_gemini_io_smoke passed")
