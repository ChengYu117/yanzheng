"""Import and validate raw Claude Code LLM JSON outputs."""
...
def parse_llm_json_file(path: str | Path) -> Any:
    return parse_llm_json_text(Path(path).read_text(encoding="utf-8"))
...
def parse_prediction_file(path: str | Path) -> list[dict[str, Any]]:
    return normalize_prediction_payload(parse_llm_json_file(path))
    ...
    *,
    ...
    for task in tasks:
        ...
        try:
            ...
            if expected.empty:
                raise ValueError("answer key missing for task")
            expected_id_list = expected["sample_id"].astype(str).tolist()
            predicted_id_list = [row["sample_id"] for row in predictions]
            duplicate_expected = sorted(
                sample_id for sample_id in set(expected_id_list) if expected_id_list.count(sample_id) > 1
            )
            duplicate_predicted = sorted(
                sample_id for sample_id in set(predicted_id_list) if predicted_id_list.count(sample_id) > 1
            )
            if duplicate_expected:
                raise ValueError(f"duplicate sample_id in answer key: {duplicate_expected}")
            if duplicate_predicted:
                raise ValueError(f"duplicate sample_id in predictions: {duplicate_predicted}")
            expected_ids = set(expected_id_list)
            predicted_ids = set(predicted_id_list)
            missing = sorted(expected_ids.difference(predicted_ids))
            extra = sorted(predicted_ids.difference(expected_ids))
            if len(predictions) != len(expected) or missing or extra:
                raise ValueError(
                    "sample_id mismatch; "
                    f"expected_count={len(expected)}, predicted_count={len(predictions)}, "
                    f"missing={missing}, extra={extra}"
                )
            expected_by_id = expected.set_index(expected["sample_id"].astype(str), verify_integrity=True)
            for pred in predictions:
                key_row = expected_by_id.loc[pred["sample_id"]]
                row = {
                    "task_id": task_id,
                    "packet_id": task.get("packet_id"),
                    "latent_idx": task.get("latent_idx"),
                    "explanation_task_id": task.get("explanation_task_id"),
                    "raw_kind": raw_kind,
