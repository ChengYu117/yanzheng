...
def build_scorer_prompt(feature_id: str, explanation: dict[str, Any], heldout: list[dict[str, Any]]) -> str:
    frozen={k:explanation[k] for k in ("short_name","contrastive_explanation","necessary_or_characteristic_condition","insufficient_conditions","surface_or_linguistic_hypothesis","behavioral_or_discourse_hypothesis","primary_explanation","explanation_type","possible_confounds","limitations")}
...
def make_scorer_tasks(*, output_dir: str | Path) -> dict[str, Any]:
    ...
    write_jsonl(output/"scorer"/"tasks.jsonl",tasks); result={"n_tasks":len(tasks)}; write_json(output/"scorer"/"task_manifest.json",result); return result
...
def validate_scorer_and_score(*, output_dir: str | Path) -> dict[str, Any]:
    ...
    for task in tasks:
        ...
        truth=packs[task["feature_id"]]["heldout_samples_private"]; y=np.asarray([float(x["true_activation"]) for x in truth]); p=np.asarray([float(x["predicted_feature_score"]) for x in row["predictions"]]); strata=[x["stratum"] for x in truth]
...
__all__=["PILOT_LATENTS","SamplingConfig","build_packets","build_full_packets","build_pilot_packets","load_stable_core_latents","make_explainer_retry_tasks","make_scorer_tasks","run_stage","validate_explanations","validate_scorer_and_score"]
