...
| P2 | 找到与标签相关的 SAE latents，并通过 AUC@K、repeated split-half 和 bootstrap CI 筛出 `stable_core` | P3 的候选 latent 来源 |
...
新的 P3 dry-run 默认应读取 `stable_core` latent set。已经生成的 `p3_feature_cards` 目录是旧 Top20 快照，保留作历史对照；后续稳定流程应使用单独输出目录，避免覆盖旧产物。

| 输入 | 路径 | 作用 |
|---|---|---|
| stable core latent 表 | `outputs/cross_val/stable_topk_selection/stable_topk_latent_set.csv` | 后续默认 P3 latent set；筛选条件为 full-data TopK*、repeated inclusion frequency 和 positive Cohen's d CI |
| K* 汇总表 | `outputs/cross_val/stable_topk_selection/stable_k_by_label.csv` | 记录每个标签的 `K_auc`、`K_stab`、`K*` 和 stable core 数量 |
...
```text
303 label-latent stable_core rows
225 unique stable_core latents
```
...
```mermaid
flowchart TD
    A["P2 stable_core latents"] --> B["P3 dry-run packet builder"]
...
这个脚本不调用 LLM。它只负责把前面阶段的 latent evidence 组织成后续 AI 可以处理的材料。若按新口径重跑，packet builder 应从 `stable_topk_latent_set.csv` 读取 `stable_core`，并从 feature store、label matrix 和 records 现场重建 top activation / contrast / random target examples，不应再依赖旧 Top20 evidence packet。
...
| `p3_input_explanation_prompts/` | Task A 要用的解释 prompt，每个 stable core label-latent 一条 |
| `p3_scoring_tasks.jsonl` | Task B 要用的评分任务，通常每个 stable core label-latent 两类任务 |
| `p3_feature_cards_dryrun.md` | dry-run 总览报告 |

旧 dry-run 快照数量如下，仅代表历史产物，不是新 stable core 口径的目标数量：
...
该历史快照中曾有 17 个 fallback packets。新 stable core 口径重跑时，应统一从 feature store、label matrix 和 records 重建 evidence packet，避免依赖旧 evidence packet 的同步状态。
...
| `rank_within_label` | 在该标签 stable core 来源排序中的 rank，通常来自 full-data positive Cohen's d rank |
...
```text
一次性让 AI 读完全部 prompts 或 scoring tasks。
```
...
第三步：检查关键数量。下表是旧 dry-run 快照的期望值；stable core 口径重跑后，应以新 manifest 中的 `n_cards`、`n_prompts` 和 `n_scoring_tasks` 为准。
