...
def _make_synthetic_eval(root: Path) -> Path:
    ...
    pd.DataFrame(
        [
            {
                "label": label,
                "n_topk_latents": min(2, len(effects.get(label, {}))),
                "topk_positive_count": min(2, len(effects.get(label, {}))),
                "topk_negative_count": 0,
                "topk_significant_count": min(2, len(effects.get(label, {}))),
                "topk_significant_share": 1.0,
                "topk_positive_ratio": 1.0,
                "topk_negative_ratio": 0.0,
                "topk_exclusive_count": min(2, len(effects.get(label, {}))),
                "topk_shared_count": 0,
                "topk_shared_ratio": 0.0,
