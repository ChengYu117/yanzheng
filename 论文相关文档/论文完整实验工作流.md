...
这些结果已经足以否定“SAE 的预测性能优于 PCA”，并支持“有针对性筛选的 SAE 子空间明显优于随机 SAE 子空间”。但最终论文主表必须补充或重算：

- 7 个 leaf 标签的 macro AUC、PR-AUC、F1 和 Balanced Accuracy；
- 逐 leaf 标签结果；
- 主要方法间 fold-level 配对差值及置信区间；
- 9 标签结果降级为父标签敏感性或附录表。
...
| 统一表示比较 | 已完成 9-label 版本 | 重算/汇总 7-leaf macro；补主要比较的配对 CI |
